...
def home(request):
    if request.user.is_authenticated:
        faculty_queryset = Faculty.objects.filter(faculty_id=request.user)
        if faculty_queryset.exists():
            faculty = faculty_queryset[0]
            context = {
    ...
    # If no faculty found or user is not authenticated, return a response without faculty-related data.
...
def view_attendance(request):
    try:
        dummy = Faculty.objects.get(faculty_id=request.user)
    except Faculty.DoesNotExist:
        return HttpResponse("Faculty not found for this user.")
